  ...
  <div ref="sale_show" class="sale_show">
  		...
  		<div v-show="!searctrue" class="sale_show-left-top">
  			...
  			<div class="wrap_num">
  				...
  				<div @click="sell_main_show_click('1')" class="wrap_detail pointer">
  					<p class="flex_one">自运营</p>
  					<p style="color: #0099ff;font-size: 18px;">{{data_tpfix_two(payment.zPayment)}}</p>
  				</div>
  				<div @click="sell_main_show_click('2')" class="wrap_detail pointer">
  					<p class="flex_one">代运营</p>
  					<p style="color: #00cc99;font-size: 18px;">{{data_tpfix_two(payment.dPayment)}}</p>
  				</div>
  			</div>
  		</div>
  		<div v-show="searctrue" class="sale_show-left-top">
  		...
  		<div style="position: relative;" :style="{height: secondtableheight + 'px'}" class="sale_show-left-second">
			...
			<div style="position: absolute;right: 30px;width: 180px;height: 40px;top: 50px;font-size: 12px;">
				...
				<p style="margin-top: 4px;">
					<span style="padding-right: 6px;">自运营：
					...
					<span style="padding-right: 6px;">代运营：
  	...
  	<div class="sale_show-right">
  		<div ref="sale_show_right_top" class="sale_show-right-top">
  			...
  			<div class="top_pie">
  				<div id="circle_one"  style="height: 100%" class="circle_wrap_big">
  				</div>  			
  			</div>
      ...
      return{
		...
		circle_three_option: {
      	...
      	other_wrap_option: {
      		...
      		//十二色备选
		    //color: ['#ff0000', '#ff3300', '#ff6600', '#ff9900','#ffff00','#99ff00','#00ff00','#00ffff','#0000ff','#6600ff','#ff00ff','ff0066'],
		    tooltip: {
		        trigger: 'axis',
		        axisPointer: {
		            type: 'shadow'
    ...
    methods:{
    	...
    	changefifteen_pay(list) {
    		...
    		if(this.searctrue){
    			...
    			} else if(this.search.searchname == '自运营'){
    				this.sell_main_show_click('1')
    			} else if(this.search.searchname == '代运营'){
    	...
    	sell_import_status() {
    		this.$http.get(`yx_sell/sell_import_status`)
	      		.then((data)=>{
    	...
    	sell_shop_type_battle_table() {
			...
			this.$http.get(`yx_sell/sell_shop_type_battle_table?type=${this.search_catagory.type ? this.search_catagory.type:1}`,{
      ...
      sell_main_show(brandId, name) {
      	...
      	this.payment.allShopPayment = ''
  			this.payment.zPayment = ''
  			this.payment.dPayment = ''


  			let brandIdsearch = this.search_catagory.brandId ? this.search_catagory.brandId : 1
		
	      	this.$http.get(`yx_sell/sell_main_show?brandId=${brandIdsearch}&type=${this.search_catagory.type ? this.search_catagory.type:1}`,{
	      		...
	      		.then((data)=>{
	      			...
	      			for (let i=0;i<data.data.sellLastYearShowVOS.length;i++) {
	      					...
	      					for (let j=0;j<data.data.sellLastYearShowVOS[i].list.length;j++) {
	      						...
	      						//this.last_year_payment[data.data.sellLastYearShowVOS[i].list[j].year - sellLastYear].data[data.data.sellLastYearShowVOS[i].month-1] = data.data.sellLastYearShowVOS[i].list[j].payment

		      				  //this.other_wrap_option.series[data.data.sellLastYearShowVOS[i].list[j].year - sellLastYear].data[data.data.sellLastYearShowVOS[i].month-1] = data.data.sellLastYearShowVOS[i].list[j].payment
              ...
              this.lastyeartablearray = tablearray
	      			...
	      			this.payment.zPayment = data.data.zPayment
	      			this.payment.dPayment = data.data.dPayment
					...
					data.data.sellMainBrandBattleShowVOS.sort(compare("brandId"))
	      			...
	      			//console.info(this.salefifteen.series)
	        			...
	        			//自运营环形图
	        			this.circle_two_option.series[0].data[0].value = data.data.zTmPayment
	        			this.circle_two_option.series[0].data[1].value = data.data.zTbPayment
	        			...
	        			//代运营环形图
	        			this.circle_three_option.series[0].data[0].value = data.data.dTmPayment
	        			this.circle_three_option.series[0].data[1].value = data.data.dTbPayment
      	...
      	sell_main_show_click(type) {
    		...
    		if (type == 1) {
    			this.search.searchname = '自运营'
    			brandId = 1
    		}
    		if (type == 2) {
    			this.search.searchname = '代运营'
    			brandId = 2
    		}
    		...
    		let searchtype = this.search_catagory.type ? this.search_catagory.type:1
      		this.$http.get(`yx_sell/sell_main_show_click?brandId=${brandId}&type=${searchtype}`)
	      		.then((data)=>{
