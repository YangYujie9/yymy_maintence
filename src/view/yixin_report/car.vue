<template>
  <div @click="close_all" class="car">
  	<div class="car-top">
		...
		<el-select v-model="search.shopIds" placeholder="请选择">
	    <el-option
		...
		<el-button @click="get_drive_report" class="monbtn" plain>查询</el-button>

		<el-button @click="export_more_shop" style="margin-left: 20px;margin-bottom: 20px" class="monbtn">导出多店开车</el-button>

		<el-button @click="export_yx_pic_link" style="margin-left: 20px;margin-bottom: 20px" class="monbtn">导出图片链接</el-button>
  	...
  	<div ref="car_wrap" v-loading = "loading" class="car-wrap">
  		<div :style="{height: (get_genshangxia_height+270)+'px'}" v-for="(list,index_list) in datalist" class="car_list">
  			...
  			<div class="car_list-wrap">
  				...
  				<div class="img_right">
  					...
  					<div class="first_title">
  						<p>类目</p>
  						<p>{{list.category ? list.category: '&nbsp;'}}</p>
  			...
  			<div :style="{height: (get_genshangxia_height+50)+'px'}" v-if="list.list && list.data_detail"  ref="car_list_bottom" class="car_list-bottom">
				...
				<div :style="{height: (get_genshangxia_height)+'px'}" style="border-top: 1px solid #DCDFE6; top: -1px;"  class="date_left">
					<ul class="pointer"  :style="{top: -list.scrolltop + 'px'}">
						...
						<li @click.stop="choose_single(list, value, index,index_list)"   v-for="(value, index) in choose_sort_list">
							{{value.cname}}
						</li>

						
						<li style="height: 72px;">备注</li>
				...
				<div   v-if="search.type != 'super'" :style="{width: car_list_bottom_width + 'px',height: (get_genshangxia_height+20)+'px'}"   @scroll="scroll_left($event,list)" class="date_content">
					...
					<ul  v-if="value.show" :style="{width: list.date_a.length*80+ 'px'}" v-for="(value, key) in choose_sort_list">


						<li v-if="value.ename != 'forecastCost'"  v-for="index in list[value.ename+'_a']">
							{{index}}{{(value.columnId == 4 || value.columnId == 7 || value.columnId == 11 || value.columnId == 12 ||value.columnId == 20 || value.columnId == 23 || value.columnId == 9 || value.columnId == 18) ? '%':''}}
						</li>
					...
					<ul style="height: 74px;" :style="{width: list.date_a.length*80 + 'px'}" >
						<li style="height: 74px;" v-for="(indexa,indexa_memo) in list.memo_a">
				...
				<div   v-if="search.type == 'super'" :style="{width: car_list_bottom_width + 'px',height: (get_genshangxia_height+20)+'px'}"   @scroll="scroll_left($event,list)" class="date_content">
					...
					<ul  v-if="value.show" :style="{width: list.date_a.length*80+ 'px'}" v-for="(value, key) in choose_sort_list_super">


						<li v-if="value.ename != 'forecastCost'"  v-for="index in list[value.ename+'_a']">
							{{index}}{{(value.columnId == 14 || value.columnId == 19 || value.columnId == 35 || value.columnId == 13 ||value.columnId == 26 || value.columnId == 29 || value.columnId == 30 || value.columnId == 35 || value.columnId == 32 || value.columnId == 9 || value.columnId == 18) ? '%':''}}
						</li>
						...
						<!--<div  v-if="list[get_en_name(value.columnId)+'_show']" class="div_qushi">
							<div class="div_qushi_title">
								{{choose_single_name}}
							</div>

							<div :id="get_en_name(value.columnId)" class="div_qushi_content">
	  ...
	  <div @click="close_all_dialog" style="height: 600px;position: relative;"  v-loading = "loading" class="car-wrap">
  		...
  		<div v-for="(list,index_list) in datalist_dialog" class="car_list">
  			<div class="car_list-top">
  				<i @click="update_fire_status(list)" :class="{iconhuo_red: list.fireStatus}" class="iconfont iconhuo pointer"></i>
  				基础信息
  			</div>
  			...
  			<div v-if="list.list" ref="car_list_bottom" class="car_list-bottom">
				...
				<div style="border-top: 1px solid #DCDFE6; top: -1px;"  class="date_left">
					<ul class="pointer"  :style="{top: -list.scrolltop + 'px'}">
						...
						<li @click.stop="choose_single_dialog(list, value, index, index_list)"  v-if="value.show" v-for="(value, index) in choose_sort_list">
							{{value.cname}}
						</li>
				...
				<div style="width: 1200px;" @scroll="scroll_left($event,list)" class="date_content">



					
					<ul   :style="{width: list.date_a.length*80+10 + 'px'}" v-for="(value, key) in choose_sort_list">
					...
					<ul style="height: 80px;" :style="{width: list.date_a.length*80 + 'px'}" >
						<li style="height: 80px;" v-for="(indexa,indexa_memo) in list.memo_a">
							<span class="pointer" style="color: green" @click="changememo(indexa_memo, list)" size="mini"  type="success">
	                        	{{indexa}}
	                      	</span>
						</li>
...
export default {
    ...
    computed: {
    	...
    	get_drive_column() {
    		let demand_status = this.getpageDict.filter((list) => {
                return list.dicType == "yx_drive_column"
            })
    ...
    methods: {
    	export_more_shop() {
    		let that= this
    		this.$sale_exportAxios.get(`yx_import/export_more_shop`,{
    	...
    	export_yx_pic_link() {
    		let that= this
    		this.$exporthttp.post(`file/export_yx_pic_link`,{},{
    	...
    	edit_drive_status(list,status) {
    		...
    		let stri = status?'停车':'开车'

    		this.$confirm(`确认该${stri}stri？`)
	          .then(_ => {
	            this.$http.post(`yx_drive/edit_drive_status`,{
    	...
    	update_drive_forecast_cost() {
    		...
    		this.$http.post(`yx_drive/update_drive_forecast_cost`,{
    			//shopIds: this.shopidchange.length ? this.shopidchange.join(',') : this.totalshops.join(',')
    	...
    	update_drive_memo() {
    		...
    		this.$http.post(`yx_drive/update_drive_memo`,{
    			//shopIds: this.shopidchange.length ? this.shopidchange.join(',') : this.totalshops.join(',')
    	...
    	edit_drive_column() {
    		...
    		this.$http.post(`yx_drive/edit_drive_column`,{
    	...
    	find_drive_column_super() {
	    		this.$http.get(`yx_drive/find_drive_column?type=super`) 
                .then((data)=>{
                	...
                	this.genshangxia_height_super = arr.length*26+86


                	//console.info(this.genshangxia_height_super)

                	this.choose_sort_list_super = arr
    	...
    	update_fire_status(list) {
    		...
    		this.$http.post(`yx_drive/update_fire_status`,{
    			//shopIds: this.shopidchange.length ? this.shopidchange.join(',') : this.totalshops.join(',')
    			wareId: list.wareId,
    			shopId: list.shopId,
    	...
    	get_one_ware_more_data(listindex) {
    		...
    		this.$http.post(`yx_drive/get_one_ware_more_data`,{
    			//shopIds: this.shopidchange.length ? this.shopidchange.join(',') : this.totalshops.join(',')
    			wareId: listindex.wareId,
    			shopId: listindex.shopId,
    			babyId: listindex.babyId,
    	...
    	find_drive_column() {
	    		this.$http.get(`yx_drive/find_drive_column?type=drive`)
                .then((data)=>{
                	let arr = []
                	...
                	this.genshangxia_height = arr.length*26+86

                	this.choose_sort_list = arr

                	//console.info(this.choose_sort_list)
    	...
    	get_more_shop_data(wareId) {
    		this.more_show_dialogVisible = true
	    		this.$http.post(`yx_drive/get_drive_report`,{
	    			shopIds: '',
	    			wareId: wareId,
    	...
    	get_drive_report() {
    		...
    		this.$http.post(`yx_drive/get_drive_report`,{
    			//shopIds: this.shopidchange.length ? this.shopidchange.join(',') : this.totalshops.join(','),
    			shopIds: this.search.shopIds ? this.search.shopIds : this.search.wareId ? '' : this.search.shopIdsfirst,
    	...
    	get_shop_by_brand() {
    		...
    		this.$http.get(`yx_shop/show_shop`)
	          .then((data)=>{
	              ...
	              this.search.shopIdsfirst = arr[0].value
