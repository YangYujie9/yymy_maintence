  			...
  			<div class="wrap_white-bottom-top">
  				<div class="wrap_white-bottom-top-title">
	  				...
	  				<div class="wrap_white-bottom-top-title-right">
	  					...
	  					<span class="pointer" @click="$emit('pagechange','/sale_report/sale_detail')" style="padding-left: 20px">查看明细</span>
...
export default {
    ...
    methods: {
    	...
    	edit_sell_column() {
    		...
    		this.$http.post(`sell/edit_sell_column`,{
    	...
    	sell_total() {
    		...
    		this.$http.post('sell/sell_total',{
    	...
    	get_sell_pic() {
    		...
    		//console.info(this.$refs.checkshopid.getCheckedNodes())


    		
		
    		this.$http.post('sell/get_sell_pic',{
                start: this.search.start[0].getTime().toString().substr(0, 10),
                end: this.search.start[1].getTime().toString().substr(0, 10),
                shopIds: this.shopidchange.length ? this.shopidchange.join(',') : this.totalshops.join(','),
              })
                ...
                for (let i=0; i<data.data.length; i++) {
                	...
                	for (let j=0;j<this.option.series.length;j++) {
                		...
                		this.option.series[j].data.push({
                			value: data.data[i][dataa[this.option.series[j].name]+'1'] ? data.data[i][dataa[this.option.series[j].name]+'1'].toFixed(2) : data.data[i][dataa[this.option.series[j].name]],
                			//value:  data.data[i][dataa[this.option.series[j].name]],
