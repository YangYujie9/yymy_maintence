  				...
  				<div>
  					...
  					<p style="margin-top: 20px;">
				        ...
				        <div class="el-checkbox-group">
				        	...
				        	<span v-if="list.choose && list.children" v-for="list in shop_brand">
				        		<button  @click="lista.choose = !lista.choose" :class="{'el-button--success-a':lista.choose}" v-for="lista in list.children"  class="el-button-s" >
						    		<span>{{lista.label}}</span>
						    	</button>
						    </span>
					...
					<div style="margin-top: 0px;">
				        	...
				        	<el-button @click="compare_to_shop" class="monbtn">搜索</el-button>
...
import echarts from 'echarts'
  export default {
      ...
      ...mapGetters([
    	...
    	shopidchange() {
    		...
    		for (let i=0;i<this.shop_brand.length;i++) {
    			if (this.shop_brand[i].choose && this.shop_brand[i].children) {
    				for (let j=0;j<this.shop_brand[i].children.length;j++) {
    					if(this.shop_brand[i].children[j].choose) {
    						arr.push(this.shop_brand[i].children[j].value)
    					}
    				}
    			}
    		}
    ...
    methods:{
      ...
      find_sell_column() {
        this.$http.get(`sell/find_sell_column`)
                .then((data)=>{
      ...
      show_circle(key,value) {
    	...
    	export_sell_report() {
    		let that = this
          this.$sale_exportAxios.post(`sell/export_sell_report`,{
    	...
    	chearall() {
    		for (let i=0;i<this.shop_brand.length;i++) {
    			...
    			if (this.shop_brand[i].children) {
    				for (let j=0;j<this.shop_brand[i].children.length;j++) {
    					this.shop_brand[i].children[j].choose = false
    				}
    			}
    		}
    	...
    	compare_to_shop() {
    		...
    		this.$http.post('sell/compare_to_shop',{
                start: this.search.start[0].getTime().toString().substr(0, 10),
                end: this.search.start[1].getTime().toString().substr(0, 10),
                shopIds: this.shopidchange.length ? this.shopidchange.join(',') : this.totalshops.join(',')
              })
